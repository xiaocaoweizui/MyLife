...
namespace MyLift.Tests
{
    public abstract class MyLiftTestBase : AbpIntegratedTestBase<MyLiftTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MyLiftTestBase()
        {
        ...
        protected void UsingDbContext(Action<MyLiftDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyLiftDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyLiftDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyLiftDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyLiftDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyLiftDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyLiftDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyLiftDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyLiftDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyLiftDbContext, Task<T>> func)
        {
            T result;
